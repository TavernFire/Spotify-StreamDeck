...
$pluginProjectFile = "$basePath\tv.tavernfire.spotify.csproj"
...
Write-Host "targetFramework : $targetFrameworkName`n"
...
Write-Host "bindir: $bindir`n"

# Make sure we actually have a directory/build to deploy
If (-not (Test-Path $bindir)) {
  Write-Error "The output directory `"$bindir`" was not found.`n You must first build the `"tv.tavernfire.spotify`" project before calling this script.";
  exit 1;
}
...
Write-Host "Killing: $pluginName`n"
